...
* **Command Encoding:** Use `dispatch_workgroups(groups_x, batch_size, 1)` where `groups_x` covers N/2 elements at 256 threads per group (`(n/2).div_ceil(256)`). The Y dimension carries the batch index into the kernel.
